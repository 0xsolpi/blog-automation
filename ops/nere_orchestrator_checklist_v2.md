...
- 트리거 5종(키워드별 분기):
  ...
  4) `ㅎㄷ.<제품쿠팡링크>` → **Luffy 생략**, Ace가 링크 기준 동일 제품 확인/검증 후 진행(발행 카테고리 ID=3)
  5) `sns.<제품쿠팡링크>` → **Luffy 생략**, Ace가 링크 기준 동일 제품 확인/검증 후 진행(발행 카테고리 ID=9, 이슈 신제품 완화모드)
...
- 라우팅 규칙:
  ...
  - `ㄹㅋ.<제품쿠팡링크>`: Ace 직행(링크 기준 제품 검증, 발행 카테고리 ID=1)
  - `ㅎㄷ.<제품쿠팡링크>`: Ace 직행(링크 기준 제품 검증, 발행 카테고리 ID=3)
  - `sns.<제품쿠팡링크>`: Ace 직행(링크 기준 제품 검증, 발행 카테고리 ID=9, 리뷰/평점/베스트리뷰 수량 하드게이트 완화)
- 승인 게이트 규칙(선장님 최신 지시):
  - 네레 판정이 `✅ 승인 권고`면 선장님 추가 승인 질의 없이 Robin 즉시 실행
  - 네레 판정이 `⚠️ 조건부` 또는 `❌ 반려`일 때만 선장님에게 승인/보류/반려 질의
- 조건부면 Nami 재수정 지시
- 반려면 run 종료
