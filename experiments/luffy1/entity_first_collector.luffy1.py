...
MODEL_RULES_PATH = ROOT / 'configs' / 'model_patterns.json'
...
BEAUTY_CATEGORIES = {'쿠션','선크림','앰플'}
BEAUTY_TERMS = {'화장품','쿠션','립','향수','선크림','앰플','마스크팩','스킨케어','헤어에센스','바디미스트'}
BEAUTY_BRANDS = {'헤라','라네즈','설화수','닥터지','에이지투웨니스','AGE20S','AGE20\'S','클리오','롬앤','이니스프리','미샤','에스쁘아'}
...
def load_model_patterns(path: Path) -> dict:
    default = {
        'brand_patterns': {
            '삼성': [r'\bNT\d{3,}[A-Z0-9-]*\b', r'\bSM-[A-Z0-9-]{3,}\b', r'\bRWP\d{4,}[A-Z0-9-]*\b'],
            'LG': [r'\b(?:WD|WU|GX|ZD|16ZD|14ZD)\d{2,}[A-Z0-9-]*\b'],
            '로보락': [r'\bS\d{2,}[A-Z0-9-]*\b', r'\bQ\d{1,}[A-Z0-9-]*\b'],
            '샤오미': [r'\bX\d{2,}[A-Z0-9-]*\b', r'\bBHR\d{3,}[A-Z0-9-]*\b'],
            '필립스': [r'\b(?:DST|HX|S\d{3,})[A-Z0-9-]*\b'],
            '애플': [r'\bA\d{4}\b', r'\bM\d{1,2}\b'],
        }
    }
    if not path.exists():
        return default
    try:
        ...
        bp = loaded.get('brand_patterns', {}) if isinstance(loaded, dict) else {}
        if isinstance(bp, dict) and bp:
            default['brand_patterns'].update(bp)
    except Exception:
        pass
    return default


def extract_model_with_rules(text: str, brand: str, model_rules: dict) -> str:
    t = normalize(text).upper()
    patterns = (model_rules or {}).get('brand_patterns', {}).get(brand, [])
    for pat in patterns:
        try:
            m = re.search(pat, t)
        except re.error:
...
def normalize_beauty_model(text: str) -> str:
    t = normalize(text).upper()
    # common shade tokens only (SPF는 모델로 취급하지 않음)
    m = re.search(r'\b(\d{2}[NC]\d)\b', t)  # 21N1, 23C1
    if m:
        return m.group(1)
    # cushion line names
...
def main():
    ...
    model_rules = load_model_patterns(MODEL_RULES_PATH)
    ...
    for cat in SHOP_CATEGORIES:
        ...
        for it in items[:10]:
            ...
            # 화장품 카테고리는 화장품 브랜드만 허용
            if cat in BEAUTY_CATEGORIES and brand not in BEAUTY_BRANDS:
                continue
            model_raw = extract_model_with_rules(title, brand, model_rules) or extract_model(title)
            model=normalize_model(model_raw, brand)
    ...
    for celeb in celeb_pool[:12]:
        for term in PRODUCT_TERMS[:12]:
            ...
            for row,src in [(x,'naver_news') for x in news] + [(x,'naver_blog') for x in blogs]:
                ...
                # 화장품 맥락(term)에서는 화장품 브랜드만 허용
                if term in BEAUTY_TERMS and brand not in BEAUTY_BRANDS:
                    continue
                model_raw = extract_model_with_rules(comb, brand, model_rules) or extract_model(comb)
                model = normalize_model(model_raw, brand)
                if not valid_entity(brand, model):
                    continue
                k = entity_key(brand, model)
